...
$ShowTotal=0;
 ...
 $QUE ="select ID,Username,ROUND(SC,6) SC,
		(select count(*) from PublicFunctions where UserID=U.ID) 'Functions created',
		(select count(*) from Instances where Starter=U.ID) 'Instances started',
		(select count(*) from Instances where Worker=U.ID) 'Instances worked',
		ROUND(IFNULL((select sum(Duration) from Instances where Worker=U.ID),0)/1000,3) 'Time worked (s)'
		from Users U
		order by SC Desc
		";
  ...
  <div class="content-wrapper">
	...
	<button type="button" class="btn btn-danger" id="Sponsor">Send 1 SC</button>
		...
		$("#Sponsor").click(function()
		{
			...
			$.post( "phps/Operations.php", {Operation: "SponsorUser",ID: L(data,"ID")}, function( data )
			{
				//console.log(data);
				location.reload();
			});
