...
function SetStyle()
{
	...
	.selector('edge')
		  .style({
			...
			{
				var ToAdd;
				if(EdgeSizes[ele.id()]!=undefined)
					ToAdd=EdgeSizes[ele.id()];
				else
					ToAdd="0";
	...
	//todo not compatible sa stearga muchia respectiva SAU sa iti modifice destinatia (daca e variabila de exemplu)
		...
		$.post( "../phps/GetPublicFunctions.php", function( data )
		{
			...
			$.post( "../phps/GetDefaultFunctions.php", function( data )
			{
				...
				// SetStyle();
				$.post( "../phps/Operations.php", {Operation: "GetDoneJson", ID:IDView}, function( data )
				{
					console.log(data);
					EdgeSizes=[];
					var temp=data.trim().split(",");
					for(var a=0;a<temp.length;a++)
					{
						...
						EdgeSizes[temp2[0]]=temp2[1];
					}
					
					$.post( "phps/Operations.php", {Operation: "GetFunctionJson",AIDI: IDF}, function( data )
					{
